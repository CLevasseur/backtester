...
mod test {
    ...
    fn update_market_order() {
        ...
        let updates = market_simulation.update_orders(
            vec![&order].into_iter(),
            &symbol_id,
            &Ohlcv::new(Utc.ymd(2016, 1, 3).and_hms(17, 0, 0), 0., 0., 0., 0., 1)
        );
    ...
    fn update_limit_long_order() {
        ...
        let updates = market_simulation.update_orders(
            vec![&executed_order].into_iter(),
            &symbol_id,
            &Ohlcv::new(Utc.ymd(2016, 1, 3).and_hms(17, 0, 0), 0., 0., 99., 0., 1)
        );
    ...
    fn update_limit_short_order() {
        ...
        let updates = market_simulation.update_orders(
            ...
            &symbol_id,
            &Ohlcv::new(Utc.ymd(2016, 1, 3).and_hms(17, 0, 0), 0., 101., 0., 0., 1)
        );
        let expected: HashMap<OrderId, OrderStatus> = [(executed_order.id().clone(), OrderStatus::Filled(executed_order.quantity()))].iter().cloned().collect();
        assert_eq!(updates, expected);
    }
